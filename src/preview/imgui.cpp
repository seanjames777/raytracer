...
void ImGui::ColorPicker(const char *label, float col[3])
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    if (ColorButton(ImVec4(col[0], col[1], col[2], 1)))
    {
        OpenPopup("picker");
    ...
    if (BeginPopup("picker"))
    {
        ...
        ImVec2 size(w_full, w_full);

        ImRect bb(GetCursorScreenPos(), window->DC.CursorPos + size);
        //ItemSize(bb);

        if (!ItemAdd(bb, NULL))
            return;

        float sv_sz = w_full - style.ItemInnerSpacing.x - square_sz;

        ImVec2 leftSz(sv_sz, sv_sz);
        ImVec2 rightSz(square_sz, sv_sz);

        float hsv[3];
        ColorConvertRGBtoHSV(col[0], col[1], col[2], hsv[0], hsv[1], hsv[2]);

        ImVec4 rgb0, rgb1;
        ColorConvertHSVtoRGB(hsv[0], 0, 1, rgb0.x, rgb0.y, rgb0.z);
        ColorConvertHSVtoRGB(hsv[0], 1, 1, rgb1.x, rgb1.y, rgb1.z);
        rgb0.w = 1;
        rgb1.w = 1;

        ImU32 c0 = GetColorU32(rgb0);
        ImU32 c1 = GetColorU32(rgb1);

        window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Min + leftSz, c0, c1, c1, c0);
        window->DrawList->AddRectFilledMultiColor(bb.Min, bb.Min + leftSz, ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 0), ImColor(0, 0, 0, 255), ImColor(0, 0, 0, 255));

        ImVec2 hueMin(bb.Min.x + leftSz.x + style.ItemInnerSpacing.x, bb.Min.y);
        ImVec2 hueMax(hueMin.x + rightSz.x, hueMin.y + rightSz.y);

        for (int i = 0; i < 6; i++)
        {
            ImVec2 min = hueMin;
            min.y += rightSz.y * i / 6.0f;

            ImVec2 max = min;
            max.x += rightSz.x;
            max.y += rightSz.y / 6.0f;

            float h0 = (float)i / 6.0f;
            float h1 = (float)(i + 1) / 6.0f;

            ImVec4 rgb0, rgb1;
            ColorConvertHSVtoRGB(h0, 1, 1, rgb0.x, rgb0.y, rgb0.z);
            ColorConvertHSVtoRGB(h1, 1, 1, rgb1.x, rgb1.y, rgb1.z);
            ...
            window->DrawList->AddRectFilledMultiColor(min, max, c0, c0, c1, c1);
        }
        ...
        InvisibleButton("sv_picker", leftSz);

        if (IsItemActive())
        {
            ImVec2 pos(io.MousePos.x - bb.Min.x, io.MousePos.y - bb.Min.y);

            if (pos.x < 0)
                pos.x = 0;
            else if (pos.x >= leftSz.x - 1)
                pos.x = leftSz.x - 1;

            if (pos.y < 0)
                pos.y = 0;
            else if (pos.y >= leftSz.y - 1)
                pos.y = leftSz.y - 1;

            hsv[1] = pos.x / (leftSz.x - 1);
            hsv[2] = 1 - pos.y / (leftSz.y - 1);

            // TODO: changed = true;
        }

        ImVec2 crossPos(bb.Min.x + hsv[1] * (leftSz.x - 1), bb.Min.y + (1 - hsv[2]) * (leftSz.x - 1));
        window->DrawList->AddLine(ImVec2(crossPos.x - 7, crossPos.y), ImVec2(crossPos.x - 2, crossPos.y), IM_COL32_WHITE);
        window->DrawList->AddLine(ImVec2(crossPos.x + 2, crossPos.y), ImVec2(crossPos.x + 7, crossPos.y), IM_COL32_WHITE);
        window->DrawList->AddLine(ImVec2(crossPos.x, crossPos.y - 7), ImVec2(crossPos.x, crossPos.y - 2), IM_COL32_WHITE);
        window->DrawList->AddLine(ImVec2(crossPos.x, crossPos.y + 2), ImVec2(crossPos.x, crossPos.y + 7), IM_COL32_WHITE);

        SetCursorScreenPos(hueMin);
        InvisibleButton("h_picker", rightSz);

        if (IsItemActive())
        {
            ImVec2 pos(io.MousePos.x - hueMin.x, io.MousePos.y - hueMin.y);

            if (pos.y < 0)
                pos.y = 0;
            else if (pos.y >= rightSz.y - 1)
                pos.y = rightSz.y - 1;

            hsv[0] = pos.y / (rightSz.y - 1);

            // TODO: changed = true;
        }

        ImVec2 linePos(hueMin.x, hueMin.y + hsv[0] * (rightSz.y - 1));
        window->DrawList->AddLine(ImVec2(linePos.x - 1, linePos.y), ImVec2(linePos.x + rightSz.x + 1, linePos.y), IM_COL32_WHITE);
        ...
        ColorConvertHSVtoRGB(hsv[0], hsv[1], hsv[2], col[0], col[1], col[2]);

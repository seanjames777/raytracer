    ...
    Raytracer       * raytracer) const
{
    ...
    for (int i = 0; i < SAMPLES * SAMPLES; i++) {
        ...
        vec3 dir = sample - interp.position;
        ...
        vec3 light = kd * ld * d1 * d2 * pdf * (1.0f / (dist * dist));

        Ray shadowRay(
            interp.position + dir * 0.001f,
            dir,
            light * sampleWeight * ray.weight,
            dist,
            ray.px,
            ray.py,
            ray.depth,
    ...
    //    raytracer->emitGlossyReflection(rayBuff, interp.position, interp.normal, ray.direction, ray, vec3(0.1f));
    //}

    float k_diff = 0.9f;
    float k_spec = 0.02f;
    float specpow = 80.0f;
    vec3 lp = vec3(6.0f, 4.0f, -1.5f);
    float lr = 30.0f;

    vec3 n = normalize(interp.normal);
    vec3 p = interp.position;
    ...
    if (ray.depth < 8) {
        ...
        vec2 jittered_2d[BOUNCE_SAMPLES * BOUNCE_SAMPLES];
        vec3 bounce_samples[BOUNCE_SAMPLES * BOUNCE_SAMPLES];
    ...
    if (ray.depth < 2) {
        ...
        vec2 jittered_2d[AO_SAMPLES * AO_SAMPLES];
